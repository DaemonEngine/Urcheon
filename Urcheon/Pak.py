...
class Builder():
	def __init__(self, source_dir, build_dir, game_name, map_profile, auto_actions=False):
		...
		self.action_list = ActionList(source_dir, game_name)
		...
		# do merge_bsp after copy or compile, this way copied or produced bsp can be modified
		...
		# Actions that can't be parallelized
		self.linear_actions = []
		# there can be multiple calls per produced file, it can't be done in parallel
		self.linear_actions.append("merge_bsp")
	...
	def build(self):
		...
		for action in self.builder_name_dict.keys():
			for file_path in self.action_list.active_action_dict[action]:
				...
				if action in self.linear_actions:
...
def main(stage=None):
	...
	if args.update:
		action_list = ActionList(args.source_dir, args.game_profile)
